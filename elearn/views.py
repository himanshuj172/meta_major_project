...
def create_user(request):           # create admin
    if request.method == 'POST':
        ...
        a = User(first_name=first_name, last_name=last_name, password=password, username=username, email=email, is_admin=True)
        a.save()
...
def acreate_profile(request):
    if request.method == 'POST':
        ...
        Profile.objects.filter(id = user_id).create(user_id=user_id, phonenumber=phonenumber, first_name=first_name, last_name=last_name, bio=bio, birth_date=birth_date, avatar=avatar, city=city, country=country, hobby=hobby)
...
class QuizCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        quiz.save()
        messages.success(self.request, 'Quiz created, Add questions.')
...
def question_add(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            question.quiz = quiz
            question.save()
            messages.success(request, 'You may add answers to the questions')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                formset.save()
                formset.save()
            messages.success(request, 'Questions and Answers saved successfully.')
            return redirect('quiz_change', quiz.pk)
    else:
...
class QuizResultsView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_taken_quizzes = quiz.taken_quizzes.count()
...
def lcreate_profile(request):
    if request.method == 'POST':
        ...
        Profile.objects.filter(id = user_id).create(user_id=user_id, bio=bio, first_name=first_name, last_name=last_name, phonenumber=phonenumber, hobby=hobby, city=city, country=country, birth_date=birth_date, avatar=avatar)
        messages.success(request, 'Profile was created successfully')
